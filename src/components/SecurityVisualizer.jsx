...
import discordLogo from '../assets/Discord_Logo.png?url';
import spotifyLogo from '../assets/Spotify_Logo.png?url';
import valorantLogo from '../assets/Valorant_Logo.png?url';
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: step === 2 || step === 3 ? 'radial-gradient(circle, rgba(239, 68, 68, 0.15) 0%, transparent 60%)' :
                    step === 4 ? 'radial-gradient(circle, rgba(52, 211, 153, 0.15) 0%, transparent 60%)' :
      ...
      <div style={{ position: 'absolute', right: '1.5rem', top: '50%', transform: 'translateY(-50%)', zIndex: 30, opacity: hasPlayed ? 1 : 0, transition: 'all 0.8s' }}>
        <button onClick={handleNext} disabled={step === 4} style={sideBtnStyle(step < 4, true)}>
        ...
        <div style={{ position: 'relative', width: '18%', transform: hasPlayed ? 'translateX(0)' : 'translateX(-100px)', opacity: hasPlayed ? 1 : 0, transition: 'all 0.8s ease' }}>
          {apps.map((app, i) => (
            <div key={app.id} style={{
              position: 'absolute', top: app.top, left: 0, right: 0, transform: `translateY(-50%) ${(step === 1 && animStage === i+1) || (step === 3 && animStage === i+1) || (step === 4 && (animStage === i*2+1 || animStage === i*2+2)) ? 'scale(1.05)' : 'scale(1)'}`,
              background: 'rgba(15, 23, 42, 0.9)', border: `2px solid ${app.color}`, borderRadius: '10px', padding: '0.6rem 0', textAlign: 'center',
              boxShadow: (step === 1 && animStage > i) || (step === 3 && animStage > i && i < 2) || (step === 4 && animStage > i*2) ? `0 0 15px ${app.color}66` : 'none',
        ...
        <div style={{ width: '22%', transform: step === 4 ? 'translateY(0)' : 'translateY(-50px)', opacity: step === 4 ? 1 : 0, pointerEvents: step === 4 ? 'auto' : 'none', transition: 'all 0.6s' }}>
          <div style={{ background: 'linear-gradient(180deg, #1e293b 0%, #0f172a 100%)', border: '2px dashed #a78bfa', borderRadius: '12px', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {[...Array(10)].map((_, i) => {
              ...
              if (step === 4) {
                if ([7, 8, 9].includes(i)) { owner = { name: 'Chrome', color: '#facc15' }; isDrawingNow = false; }
              ...
              return (
                <div key={`vram-${i}`} className={isDrawingNow ? 'fade-block-delayed' : 'fade-block'} style={{ 
            ...
            {[...Array(10)].map((_, i) => {
              ...
              return (
                <div key={`pram-${i}`} className={isDrawingNow ? 'fade-block-delayed' : 'fade-block'} style={{ 
                  ...
                  animation: doShake ? 'shake 0.4s infinite' : '', transition: 'background 0.3s'
                }}>
                  {owner && !isCorrupted && (
                    <div style={{ color: '#fff', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px' }}>
                      {owner.name}<span className="loading-dots"></span>
      ...
      {/* FOOTER TEXT */}
      <div style={{ marginTop: '2rem', padding: '1.2rem', borderRadius: '12px', height: '110px', display: 'flex', alignItems: 'center', zIndex: 10, background: step === 3 ? 'rgba(239, 68, 68, 0.1)' : (step === 4 ? 'rgba(52, 211, 153, 0.1)' : 'rgba(255,255,255,0.05)'), borderLeft: `4px solid ${step === 3 ? '#ef4444' : (step === 4 ? '#34d399' : '#38bdf8')}`, opacity: hasPlayed ? 1 : 0, transition: 'all 0.5s' }}>
        <p style={{ margin: 0, fontSize: '1rem', color: '#e2e8f0', lineHeight: '1.6' }}>
          ...
          {step === 4 && "With Virtual Memory, the OS gives every app an isolated 'Virtual RAM' from index 0. The hardware MMU strictly maps these sandboxes into fragmented Physical RAM behind the scenes, blocking apps from ever touching each other."}
        </p>
      </div>
      ...
      <style>{`
        ...
        .fade-block-delayed {
          opacity: 0;
          animation: fadeInBlock 0.3s ease 0.8s forwards;
        }
        .fade-block {
          opacity: 1;
        }
